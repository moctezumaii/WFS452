...
n <- nrow(CreekChub)
aged_indices <- sample(1:n, size = round(0.4 * n))
...
Look at the ALK table. Pick one length category and describe what the proportions tell you. For example: "A fish in the \_\_\_ mm length category has a \_\_\_% chance of being age \_\_\_ and a \_\_\_% chance of being age \_\_\_."
:::
...
Now, let's fit the Von Bertalanffy model. We need starting values for the parameters. The `vbStarts()` function helps us get reasonable guesses:
...
-   Is $K$ relatively high or low? What does that mean for how fast they grow?
-   What does $t_0$ represent? Is a negative value expected?
:::
...
age_seq <- seq(min(full_data$age), max(full_data$age), length.out = 100)
